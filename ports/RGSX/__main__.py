    ...
    # Popup download_result supprimé : plus de temporisation de 3s
        ...
        # Gestion de l'état loading
            ...
            if loading_step == "none":
                ...
                # Optional: disable in-app code updater via env flag
                if getattr(config, 'DISABLE_CODE_UPDATE', False):
                    logger.debug("DISABLE_CODE_UPDATE=true -> skipping OTA check")
                ...
                if is_data_empty and not getattr(config, 'DISABLE_DATA_UPDATE', False):
                    ...
                    if is_data_empty and getattr(config, 'DISABLE_DATA_UPDATE', False):
                        logger.debug("Data missing but DISABLE_DATA_UPDATE=true -> skipping data download")
                    loading_step = "load_sources"
                    config.current_loading_system = _("loading_load_systems")
                    config.loading_progress = 80.0
                    config.needs_redraw = True
